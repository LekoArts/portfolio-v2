...
- `darkThemeClass`: The generated CSS class from vanilla-extract (e.g. `const darkThemeClass = createTheme()`)
...
export const output = themeAwareStyles({ selectorMap, defaultTheme, darkThemeClass })`,
      active: true,
    },
    "/utils.ts": {
...
Make the utility function your own or create a library out of it! So far the function only supports two themes and its naming mostly assumes light/dark mode. An improved version could support `n` amount of themes for example.
