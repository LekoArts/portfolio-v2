...
type: "tutorial"
category: "JavaScript"
image: /og-images/vanilla-extract-overview.png?v=1
published: true
---
...
1. Define a theme contract and two themes. In my projects I also always begin with the themes first before doing the actual styling of the app.
1. Explanation of the different styling APIs (`style`, `styleVariants`) and external packages (Sprinkles, Recipes).
1. Summary of what you built.
1. Inspiration and more learning resources.
...
The [official docs][vanilla-extract] are also super well written, so I'd recommend having them open in another tab to reference more examples while going through this tutorial.
...
Click on the `localhost` URL to open it in your browser. You should see an unstyled page with content. When the tutorial talks about the _browser preview_, it refers to this window.
...
```tsx title=src/components/app.tsx highlight=17,50
import * as React from "react"
import { lightThemeClass, darkThemeClass } from "../styles/themes.css"
import { Stats, StatsItem } from "./stats"
import { useColorScheme } from "../hooks/use-color-scheme"
...
![Preview of the app screen both in light (top) and dark (bottom) mode. The app has "Last 30 Days" on top left, "Toggle Variant" button on top right. Below that it's a list of different statistics. At the very bottom there is a centred footer.](./style_end.jpg)
...
![A horizontal top bar with "Last 30 Days" and "Toggle Variant" button. Below that is a list of statistics items which percentages.](./sprinkles_end.jpg)
...
So, what's a `<Box>` component? It's a low-level component for binding your theme-based styles to an element. You can apply CSS properties of your design system through props to this `<Box>` component. You might have seen this in e.g. [Chakra UI](https://chakra-ui.com/docs/components/box). Here's an example usage:
...
So how can you make one of your own? Luckily a community member of vanilla-extract created [Dessert Box](https://github.com/TheMightyPenguin/dessert-box). (Side note, don't you also get hungry hearing all those food terms?)
...
And et voilà, you have your own Sprinkles-powered `<Box>` component. Read the [Dessert Box Readme](https://github.com/TheMightyPenguin/dessert-box) for more information.
...
[Recipes][recipes] allows you to create multi-variant styles and the authors were heavily inspired by [Stitches](https://stitches.dev/). It also reminds me a bit of [Chakra UI's component style](https://chakra-ui.com/docs/styled-system/component-style) API.
...
<Alert status="warning" title="Things to look out for when combining Sprinkles & Recipes">

You can use Sprinkles and Recipes together, you'll see in just a bit how. But you'll need to look out for at least two things when combining them:

- You can easily generate massive amount of CSS classes as Sprinkles' `conditions` and Recipes are not optimized for each other yet. This will hurt both build & frontend performance.
- It can get confusing when you use both of them without a system that tells you when & where to use which (they are different design paradigms after all).

I'd advise you to choose one of them (e.g. it makes more sense to you, your team likes it more, you used Tailwind/Stitches in the past, etc.) and then stick with that. This way you can't run into the above mentioned problems.

Of course, if you absolutely know what you're doing, take this advice with a grain of salt. But I think that the best advice for most folks is to choose either Sprinkles or Recipes and then stick with it.
...
```ts title=src/styles/recipes.css.ts
...
```tsx title=src/components/stats.tsx highlight=2
import * as React from "react"
...
```tsx title=src/components/stats.tsx highlight=8
// Imports
...
```tsx title=src/components/stats.tsx highlight=10-13
// Imports
...
And that's a wrap! Congrats, you now completely styled the tutorial app 🎉 Give the "Toggle Variant" button a try and switch between light and dark mode ([How to emulate `prefers-color-scheme` in DevTools](https://web.dev/prefers-color-scheme/#emulating-prefers-color-scheme-in-devtools)).
...
Believe it or not, but I still do have ideas for other blog posts about vanilla-extract. You can do so many cool things with it! I'll link them below when I have written them. In the meantime, I can highly recommend checking out the [vanilla-extract docs][vanilla-extract] as they have more examples for smaller use cases I couldn't cover here.

If you're curious how I use vanilla-extract in production, you can check out my [portfolio](https://github.com/LekoArts/portfolio-v2/tree/main/www) or [tmdb.lekoarts.de](https://github.com/LekoArts/gatsby-source-tmdb/tree/main/example). I migrated my site from Chakra UI to vanilla-extract ([PR on GitHub](https://github.com/LekoArts/portfolio-v2/pull/52)), resulting in better site performance!

Have you built something cool with this tutorial or the template? Have you converted your project to vanilla-extract? Let me know on [Twitter](https://twitter.com/lekoarts_de)!
