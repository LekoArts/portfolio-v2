...
That's where it should have arrived by now: Do not work on a single level! Create as many layers as you like, arrange them with groups and mark important layers/groups with color - and name your layers/groups! It may sound like extra work to name your layers, but this will save you stress later if you have to change something in the file again. If you are absolutely lazy, you should at least name your groups... This also includes that you should name your Photoshop files properly. Image Final Final Really Final v2 Change.psd is not a good name! In such cases just use an increasing version number like Image_v01.psd.

## Keyboard shortcuts

You'll work so much faster when you don't have to use the mouse for frequently used tools. The only thing that really helps here is: practice, practice, practice.

## Pen Tool

The Pen Tool seems a bit bulky and complicated at first, unlike a magic wand tool or something similar. But when used correctly, the Pen Tool tool gives the best results for masks. Learn to use the tool, learn to create curves. The tool is not there to set a point every 10 pixels!

## Watch out: Saturation and Sharpness

Far too often you see images that are extremely oversharpened or where the saturation of the image has been extremely increased. Use a halfway calibrated monitor (i.e. it should display colors correctly in the best case) and sharpen your image with a 100% view.
