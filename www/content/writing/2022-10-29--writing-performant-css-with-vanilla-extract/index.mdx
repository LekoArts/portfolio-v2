...
And then behind the scenes the `colorScheme` can modify the `background` and `border-color`, the `size` can modify the `font-size` and `padding`, and `variant` can modify if the button is filled or only has a border. It feels like e.g. using a `<Button>` component from a pre-built UI kit.
