...
```js title=plugins/gatsby-source-plausible/gatsby-node.js highlight=3,11-16
const fetch = require("node-fetch")
const POST_NODE_TYPE = `PlausibleTopPage`
...
```js title=plugins/gatsby-source-plausible/gatsby-node.js highlight=18-31
const fetch = require("node-fetch")
const POST_NODE_TYPE = `PlausibleTopPage`
const API_ENDPOINT = `https://plausible.io/api/v1/stats/breakdown`
...
```js title=gatsby-config.js highlight=1,7-13
